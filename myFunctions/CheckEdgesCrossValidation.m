...
function G = CheckEdgesCrossValidation(X,G,eta,numSearch)
...
for i = eta
    ...
    for e = 1:length(indedges)
        ...
        if (i > 0)
            nei = FindNeighborsUndirected(G,ii,1);
            nej = FindNeighborsUndirected(G,jj,1);
            SepSearch = SeparatorSearchSpaceNew(nei,nej,ii,jj,G,1,i);
            SepSearch = SepSearch(randperm(length(SepSearch)));
            if ~isempty(SepSearch)
                comb = combnk(SepSearch,i); % all possible separators
            else
                comb = [];
            end
            searchsz = min(numSearch,size(comb,1));
            for j = 1:searchsz % search over all possible separators
                S = comb(j,:);
...
K = 5;
n = size(X,1);
numTimes = 10; % number of times 10-fold CV is performed
bool = 0;
if isempty(S)
    for ct = 1:numTimes
