...
%%********************************************************************
   ...
   function [obj,X,y,Z,runhist,W] = sdpNALsmooth(blk,At,C,b,mu0,OPTIONS);
...
%%
   ...
   if (scale_data==2)
      for iter = 1:maxiterAlt
         par.gamma   = sig0.*mu; 
         tmp = ops(C,'-',Z); 
         rhs = Rp/sig0 + AXfun(blk,At,tmp); 
         y   = linsysolvefun(L,rhs);
         Aty = Atyfun(blk,At,y); 
         tmp = ops(C,'-',Aty); 
         W   = ops(X,'-',ops(sig0,'*',tmp)); 
         %% [X,Wn,dd,rankWp] = project(blk,W); Z  = ops(Wn,'*',1/sig0);
         [X] = project3(blk,W,par); 
         tmp = ops(X,'-',W); 
         Z  = ops(tmp,'*',1/sig0); 
         Rp = b-AXfun(blk,At,X); 
         Rd = ops(C,'-',ops(Aty,'+',Z));
...
%%
   ...
   fprintf('  %3.1e          ',sig);  
   %%fprintf(' %2.0d', rankWp(1)); 
   %%if (length(rankWp) > 1); fprintf(' %2.0d',rankWp(2)); end
...
%%
   for iter = 1:maxiter 
	 ...
	 if (use_smoothing)
         ...
         normRdsub = ops(ops(Wp,'-',Xold),'norm')/sig; 
	 const = 0.2; 
	 ...
	 idx  = find(subhist.priminf < 1.1*priminfmin); 
      ...
      %% Aty = Atyfun(blk,At,y);        
      %% Rd  = ops(C,'-',ops(Aty,'+',Z));
      normRd = ops(ops(X,'-',Xold),'norm')/sig; 
      if (use_smoothing)
         pobjadd = -sum(nblk.*mu*log(normb0/normA0));
         dobjadd = sum(nblk.*mu.*log(normC0)) + sum(nblk.*mu.*(1-log(mu0+eps)));
         logdetX = blklogdet(blk,X);
         logdetZ = ops(ops(mu,'./',par.phi),'sumlog'); 
         obj(1)  = objscale*(blktrace(blk,C,X) - mu'*logdetX + pobjadd);
         obj(2)  = objscale*(b'*y + mu'*logdetZ + dobjadd);
      else
         obj = objscale*[blktrace(blk,C,X), b'*y];  
      ...
      fprintf(' %4.1f  %3.1e %3.1e ',sum(runhist.cputime),sig,sighat); 
      %%fprintf(' %2.0f',length(par.posidx{1}));
